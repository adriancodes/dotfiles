...
description: "Use when adding regression coverage to AI-assisted code, or when the same model both wrote and reviewed a change."
license: MIT
metadata:
  origin: ECC
  source: https://github.com/affaan-m/ECC/blob/e04ea0b9cc8248686edf5ac751cadff550e162b8/skills/ai-regression-testing/SKILL.md
  revision: e04ea0b9cc8248686edf5ac751cadff550e162b8
  category: Infrastructure and Data
  summary: "ECC reference for ai regression testing; adapted for explicit local scope and evidence-backed use."
---

# Ai Regression Testing — ECC Import
...
The upstream anecdote illustrates **sandbox/production path inconsistency**; it does not establish a general frequency ranking.

## Sandbox-Mode API Testing

If the project already has a sandbox/mock mode, it may support fast isolated route checks. Do not introduce a production bypass merely for these examples.
...
## Project-Native Regression Workflow

Preserve the reported failure and existing project test conventions. Trace each affected execution mode against the same consumer-visible contract. A sandbox-only test does not establish parity with the real persistence path.

Use existing project tools only for explicitly authorized local verification. Do not add a custom harness command, make every review run a full build, install a test runner, or require a coverage percentage. A read-only persona inspects supplied results instead of executing tests.

Keep a regression case when it would fail on the actual defect; do not assert mere field presence on an empty array, pin mock echoes, or replace persistence integration coverage with a sandbox fixture.
...
- Preserve reported failures and choose the smallest authorized scenario that covers the defect
- Keep fixtures deterministic and isolated; use workload-appropriate time budgets
