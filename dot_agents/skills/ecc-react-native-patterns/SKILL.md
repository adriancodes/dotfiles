...
description: "Use when implementing React Native or Expo navigation, screen state, lists, native APIs, forms, or secure storage."
license: MIT
metadata:
  origin: ECC
  source: "https://github.com/affaan-m/ECC/blob/e04ea0b9cc8248686edf5ac751cadff550e162b8/skills/react-native-patterns/SKILL.md"
  revision: e04ea0b9cc8248686edf5ac751cadff550e162b8
  category: Web
  summary: "React Native and Expo patterns for navigation, native state, lists, APIs, and storage."
---
> Attributed third-party import from [ECC](https://github.com/affaan-m/ECC/blob/e04ea0b9cc8248686edf5ac751cadff550e162b8/skills/react-native-patterns/SKILL.md) at `e04ea0b9cc8248686edf5ac751cadff550e162b8`. Adapted for on-demand, harness-neutral use; not behaviorally evaluated or certified by this collection. See `LICENSE`.
...
Practical patterns for building production React Native apps with Expo. Covers navigation, state, data fetching, lists, styling, and native APIs. Apply these patterns directly without an external rules installation.

Libraries named below (NativeWind, Zustand/Jotai, TanStack Query) are common, well-established options shown for illustration — the patterns matter more than the specific package, and any equivalent works. Zod illustrates boundary validation; use the existing equivalent schema mechanism when present.
...
export function useCurrentLocation() {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      if (active) setState({ status: 'error', message: error instanceof Error ? error.message : 'Location unavailable' })
    })
...
// Memo may skip row renders when item props are unchanged; measure before retaining it.
...
- Virtualize expensive lists and provide a stable `keyExtractor`; measure before adding `renderItem` memoization.
...
- `ecc-react-patterns` if available; otherwise reuse React state and hook concepts without DOM-specific APIs.
- `ecc-e2e-testing` if available; otherwise use isolated fixtures and observable journeys with the existing native runner, such as Maestro or Detox.
