---
name: ecc-crosspost
description: Multi-platform content distribution across X, LinkedIn, Threads, and Bluesky. Adapts content per platform using ecc-content-engine patterns. Never posts identical content cross-platform. Use when the user wants to distribute content across social platforms.
metadata:
  origin: ECC
  source: https://github.com/affaan-m/ECC/blob/e04ea0b9cc8248686edf5ac751cadff550e162b8/skills/crosspost/SKILL.md
...
Drafting variants works from supplied source material without platform access. Actual posting requires the user's approval of the content, accounts, platforms, and timing plus an already-available authorized integration for each destination. Installation does not connect accounts, schedule jobs, or authorize publishing. If a platform API is missing, return its draft and unresolved prerequisite; never claim it was posted.
...
Optionally use `ecc-content-engine` if the source still needs shaping; otherwise extract the strongest supported claim and remove filler before applying the platform rules below.

### Step 2: Capture the Voice Fingerprint

Optionally use `ecc-brand-voice` if the source voice is not already captured; otherwise record sample-backed rhythm, compression, claim style, preferred moves, and avoided moves in a short `VOICE PROFILE`.
...
- `ecc-brand-voice` for optional reusable voice capture; Step 2 is the fallback
- `ecc-content-engine` for optional source shaping; Step 1 is the fallback
- `ecc-x-api` for optional X publishing workflows; without an authorized API, return the approved draft and state that publishing is unavailable

## Attribution

Adapted from ECC by Affaan Mustafa, revision `e04ea0b9cc8248686edf5ac751cadff550e162b8`. MIT license; see `LICENSE` in this directory. Local adaptations namespace optional references and make per-platform publishing prerequisites explicit; no effectiveness or live-integration claim is made.
