...
description: "Use when writing or reviewing PyTorch training loops, model architectures, or data loading, or when a run will not reproduce."
license: MIT
metadata:
  origin: ECC
  source: https://github.com/affaan-m/ECC/blob/e04ea0b9cc8248686edf5ac751cadff550e162b8/skills/pytorch-patterns/SKILL.md
  revision: e04ea0b9cc8248686edf5ac751cadff550e162b8
  category: Infrastructure and Data
  summary: "ECC reference for pytorch patterns; adapted for explicit local scope and evidence-backed use."
---

# Pytorch Patterns — ECC Import
...
Declare supported devices explicitly and follow the requested device contract. A portable library can select CPU/CUDA/MPS deliberately; do not silently fall back when the requested behavior requires a specific accelerator.
...
Set relevant seeds and record versions, devices, deterministic-algorithm settings, and DataLoader worker seeding. Seeds alone do not guarantee identical results across kernels, hardware, or releases.

```python
# Reproducibility controls; not a cross-platform determinism guarantee.
...
```python
# Example expects 32x32 RGB images; derive/validate dimensions for other inputs.
...
def train_one_epoch(
    ...
    for batch_idx, (data, target) in enumerate(dataloader):
        ...
        with torch.amp.autocast(device.type, enabled=scaler is not None and device.type == "cuda"):
            output = model(data)
        ...
        total_loss += loss.item() * target.size(0)  # Assumes criterion reduction="mean".
...
@torch.no_grad()  # Equivalent gradient-disabling semantics to the context manager.
def evaluate(
    model: nn.Module,
    dataloader: DataLoader,
    criterion: nn.Module,
    ...
    for data, target in dataloader:
        data, target = data.to(device), target.to(device)
        output = model(data)
        total_loss += criterion(output, target).item() * target.size(0)  # Mean reduction.
...
class ImageDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> tuple[torch.Tensor, int]:
        ...
        if self.transform is None:
            raise ValueError("a tensor-producing transform is required")
...
# num_workers=0 is valid for tiny/in-memory data and debugging; benchmark workers.
# pin_memory is primarily useful for CUDA transfers; persistent_workers requires workers > 0.
# Do not drop validation examples merely to maintain a uniform batch size.
```
...
```python
# Model/optimizer checkpoint fragment; exact resume also needs scheduler, scaler, RNG and sampler state.
def save_checkpoint(
    model: nn.Module,
    optimizer: torch.optim.Optimizer,
    epoch: int,
...
# Weights-only artifact is valid for inference, but insufficient for exact training resume.
...
# Modes trade compile time, memory and kernel tuning; no mode is universally fastest.
```
...
| `torch.amp.autocast` | Device/dtype-dependent mixed precision; measure quality and speed |
| `pin_memory=True` | Faster CPU→GPU data transfer |
| `torch.compile` | JIT compilation for speed (2.0+) |
| `weights_only=True` | Restricts deserialization; still require trusted artifacts and resource limits |
