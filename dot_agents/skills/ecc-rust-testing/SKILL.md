...
description: Use when writing Rust unit, integration, async, property, or documentation tests, benchmarks, or scoped coverage checks.
license: MIT
metadata:
  origin: ECC
  source: https://github.com/affaan-m/ECC/blob/e04ea0b9cc8248686edf5ac751cadff550e162b8/skills/rust-testing/SKILL.md
  revision: e04ea0b9cc8248686edf5ac751cadff550e162b8
  category: Rust
  summary: Rust rust testing reference adapted from ECC for bounded local work.
---
...
For an authorized edit, verify the affected behavior using the project's focused checks and distinguish observed results from unexecuted examples. When used only for advice or review, inspect source and supplied evidence without running commands.
...
7. **Check coverage** — If already configured, use cargo-llvm-cov with the project-defined coverage policy.
...
```bash
# Use the already-installed, project-approved cargo-llvm-cov tool.
cargo llvm-cov                    # Summary
cargo llvm-cov --html             # HTML report
cargo llvm-cov --lcov > lcov.info # LCOV format for CI
# Preserve the project-defined coverage gate when one exists.
```

### Coverage Targets

Use the project-defined coverage threshold and exclusions. Prioritize meaningful behavior and error-path checks; do not weaken an existing gate or add a blanket percentage.
...
test:
  ...
  steps:
    ...
    # Use the existing project-pinned toolchain with clippy and rustfmt.
    ...
    # Requires the existing project-approved cargo-llvm-cov tool.

    - name: Coverage
      run: cargo llvm-cov
    # Retain the repository-defined coverage gate, if present.
```
