...
description: "Use when implementing React drag gestures, animated text, SVG motion, custom animation hooks, or imperative sequences."
license: MIT
metadata:
  origin: ECC
  source: "https://github.com/affaan-m/ECC/blob/e04ea0b9cc8248686edf5ac751cadff550e162b8/skills/motion-advanced/SKILL.md"
  revision: e04ea0b9cc8248686edf5ac751cadff550e162b8
  category: Web
  summary: "Advanced React motion gestures, SVG, text, loaders, hooks, and imperative sequences."
---
> Attributed third-party import from [ECC](https://github.com/affaan-m/ECC/blob/e04ea0b9cc8248686edf5ac751cadff550e162b8/skills/motion-advanced/SKILL.md) at `e04ea0b9cc8248686edf5ac751cadff550e162b8`. Adapted for on-demand, harness-neutral use; not behaviorally evaluated or certified by this collection. See `LICENSE`.
...
Read [motion foundations](references/motion-foundations.md) for tokens, springs, runtime flags, reduced-motion handling, and SSR constraints. The complete [token example](examples/motion-tokens.ts) supplies the `@/lib/motion-tokens` imports below; adapt that illustrative alias to the project's existing module. Other application aliases in these recipes are illustrated in the foundation, not prerequisites to install.

These animation fragments do not supply a complete accessible dialog, drag alternative, progress announcement, or route-focus strategy. Preserve the project's semantic controls, focus management, keyboard alternatives, and status feedback. Gate transforms, including hover/tap/drag, on reduced-motion preferences and keep content visible if animation is disabled. Use existing design tokens and installed APIs rather than imposing this reference's library, preset, or device heuristic universally.

# Motion Advanced

Complex, interactive, and physics-based animation patterns.
Requires [bundled motion foundations](references/motion-foundations.md) to be set up first.
Use these when the simpler recipes are insufficient; optional `ecc-motion-patterns` covers common transitions, or use the existing application transitions directly.
...
6. **All token values are imported from [bundled motion foundations](references/motion-foundations.md).** No inline numbers.
...
`AnimatePresence`, and tokens from [bundled motion foundations](references/motion-foundations.md):
...
- Token and spring definitions → see [bundled motion foundations](references/motion-foundations.md)
...
- `ecc-motion-patterns` if available; otherwise use the patterns and bundled foundation here without installing or invoking another skill.
