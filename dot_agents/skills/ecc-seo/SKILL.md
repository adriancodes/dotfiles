...
metadata:
  origin: ECC
  source: https://github.com/affaan-m/ECC/blob/e04ea0b9cc8248686edf5ac751cadff550e162b8/skills/seo/SKILL.md
...
Read the actual pages or supplied page sources before recommending changes. Crawl evidence, field performance data, search-console data, and keyword datasets require their own available tools or user exports; mark them unverified when missing. This guide does not authorize website deployment, account connections, external submissions, or installing audit services. Local page edits require an implementation request; otherwise return findings and proposed fixes.
...
- Optional specialist SEO review: use the technical and on-page checklists above if no specialist is available; the upstream `seo-specialist` is not required or registered.
- Optional frontend guidance: follow the project's existing component and rendering patterns; the upstream `frontend-patterns` companion is not required or installed.
- Optional `ecc-brand-voice`: otherwise use supplied brand samples to preserve tone while editing.
- Optional `ecc-market-research`: otherwise source keyword and audience claims directly and label estimates.

## Attribution

Adapted from ECC by Affaan Mustafa, revision `e04ea0b9cc8248686edf5ac751cadff550e162b8`. MIT license; see `LICENSE` in this directory. Local adaptations namespace optional references and distinguish evidence-based audits from deployment; no ranking, effectiveness, or live-integration claim is made.
