...
This is a generation reference, not a runnable finished deck. Implement the controller methods and replace example values before delivery. Local invocation prerequisites in `SKILL.md` apply: browser verification is required, external font requests need opt-in, and image processing or browser persistence is optional.
...
<html lang="en">
  <head>
    ...
    <!-- Fonts: use approved Fontshare/Google Fonts URLs, supplied local fonts, or an intentional system-font treatment -->
...
4. **Inline Editing** (only if the user explicitly opts in during content discovery — skip entirely otherwise):
...
**Without explicit opt-in for inline editing, do NOT generate any edit-related HTML, CSS, or JS.**
...
If the user requested no images, skip this entirely. If images were provided, process them only when requested and the dependency is already available.

**Optional dependency:** existing Python with Pillow. Do not install it automatically; retain original images or request user-prepared assets if unavailable.
